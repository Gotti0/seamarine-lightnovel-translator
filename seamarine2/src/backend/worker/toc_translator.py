...
""".strip() + "\n\n" + ai_model_data.system_prompt
    ...
    def _translate_text_dict_chunk(self, chunk: dict[int, str], chunk_index: int):
        ...
        try:
            ...
            if translated_text_dict.keys() != chunk.keys():
                ...
                # Note: TranslateCore already retried internally, so if we are here, it might be a parsing issue or model issue.
                # We could potentially retry here, but for now let's assume failure if keys don't match.
                is_suceed = False
    ...
    def _translate_toc(self, data: list[LineData], save_path: str, original_path: str):
        ...
        try:
            self._logger.info(f"TOC Translation")
            # TranslateCore handles retries and key rotation internally
            ...
            if len(translated_lines) != len(data) or re.sub(r'^\[\d+\]\s*', '', translated_lines[-1]).strip() == "":
                self._logger.warning(
                    f"Failed To Parse Translated Response\n" + \
            ...
            self._logger.error(f"Translation Error for TOC after all retries: {e}")
            return False
